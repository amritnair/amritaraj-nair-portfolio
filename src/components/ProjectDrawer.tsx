...
const accentBar: Record<string, string> = {
  ...
  "Shot Sensei":   "from-amber-400 to-teal-400",
...
const ProjectDrawer = ({ project, onClose }: Props) => {
  ...
  const gradient = accentBar[project?.title ?? ""] ?? "from-teal-400 to-sky-400";
      ...
      {project && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-50 max-h-[90vh] overflow-y-auto surface-panel-strong shadow-2xl border-t border-border rounded-t-lg"
          >
            {/* Drag handle */}
            <div className="sticky top-0 bg-sky-50/95 backdrop-blur-sm pt-5 pb-4 px-6 z-10 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <button onClick={onClose}
                  className="p-2 hover:bg-muted transition-colors text-muted-foreground hover:text-foreground rounded-lg"
            ...
            <div className="px-6 pb-10 space-y-8 max-w-2xl mx-auto">
              {/* Hero image */}
              <div className="overflow-hidden aspect-video bg-muted border border-border rounded-lg shadow-sm">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.technologies.map((t) => (
                    <Badge key={t} variant="outline" className="text-xs mono text-muted-foreground rounded-full">
                      {t}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <Button asChild className="btn-primary text-white border-0 font-mono font-semibold flex-1 sm:flex-none rounded-lg">
                ...
                {project.demoUrl && (
                  <Button asChild variant="outline" className="hover:border-primary/40 flex-1 sm:flex-none font-mono rounded-lg">
