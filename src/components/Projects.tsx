...
const meta: Record<string, { tag: string; tagColor: string; accentBar: string; demoUrl?: string }> = {
  "AlphaForge":    { tag: "Live Product",  tagColor: "bg-emerald-100 text-emerald-700 border-emerald-200", accentBar: "bg-emerald-400" },
  "Shot Sensei":   { tag: "Award Winner",  tagColor: "bg-amber-100 text-amber-700 border-amber-200",       accentBar: "bg-amber-400",   demoUrl: "https://www.youtube.com/watch?v=v3SNs0O3G5g" },
  "ClinicalHours": { tag: "AI-Powered",    tagColor: "bg-sky-100 text-sky-700 border-sky-200",             accentBar: "bg-sky-400"   },
};
...
const TiltCard = ({ project, index }: { project: typeof PROJECTS[0]; index: number }) => {
  ...
  const m = meta[project.title] ?? { tag: "Featured", tagColor: "bg-orange-100 text-orange-700 border-orange-200", accentBar: "bg-orange-400" };
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="group relative rounded-2xl border border-border bg-card overflow-hidden card-glow shadow-sm"
      >
        ...
        {/* Content */}
        <div className="p-5">
          <h3 className="display font-bold mb-2 text-foreground group-hover:text-primary transition-colors duration-200">
            {project.title}
          </h3>
      ...
      <div className="container mx-auto px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {featured.map((p, i) => <TiltCard key={p.title} project={p} index={i} />)}
          </div>
