...
const Marquee = () => {
  ...
  return (
    <div className="relative z-10 py-5 border-y border-border overflow-hidden backdrop-blur-sm" style={{ background: "hsl(195 55% 91% / 0.78)" }}>
      ...
      <div className="marquee-track">
        {doubled.map((item, i) => (
          <span key={i} className="mono text-[11px] text-muted-foreground whitespace-nowrap px-8 flex items-center gap-6">
            {item}
            <span className="text-xs" style={{ color: "hsl(200 70% 48%)" }}>·</span>
          </span>
        ))}
      </div>
    </div>
...
const Index = () => (
  <div className="min-h-screen bg-background relative overflow-hidden noise dot-grid aurora-stage">
    ...
    <footer className="relative z-10 border-t border-border py-10 backdrop-blur-sm" style={{ background: "hsl(28 62% 83% / 0.88)" }}>
